-- Create brand_templates table for storing user templates
...
CREATE OR REPLACE FUNCTION public.update_brand_templates_updated_at()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = now();
  RETURN NEW;
...
FOR EACH ROW
EXECUTE FUNCTION public.update_brand_templates_updated_at();